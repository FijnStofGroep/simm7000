...
protected:
    bool initImpl(const char *pin = NULL)
    {
        ...
        if (!testAT())
        {
            DBG(GF("### TinyGSM testAT() faild => No \"AT\" response."));
            return false;
        }

        DBG(GF("### TinyGSM Set Echo Off"));
        ...
        DBG(GF("### Modem Name:"), getModemName());

        // Enable Local Time Stamp for getting network time
        sendAT(GF("+CLTS=1"));
        if (waitResponse(10000L) != 1)
        {
            DBG(GF("### TinyGSM ** Enable Local Time Stamp for getting network time faild. **"));
            return false;
        }
        ...
        if (waitResponse() != 1)
        {
            DBG(GF("### TinyGSM ** Enable battery checks faild. **"));
            return false;
        }
        ...
        else
        {
            ...
            // return true
            String tmp = ((ret = SimStatus::SIM_READY) ? GF("SIM READY") : GF("SIM LOCKED"));
            DBG(GF("### TinyGSM Sim Status: ") + tmp);
                        ...
                        GsmConstStr r5 = NULL)
    {
        ...
        do
        {
            ...
            while (stream.available() > 0)
            {
                ...
                {
                    index = 1;
                    DBG("### Received Data(r1) from Modem:", data);
                    goto finish;
                }
                else if (r2 && data.endsWith(r2))
                {
                    index = 2;
                    DBG("### Received Data(r2) from Modem:", data);
                    goto finish;
                }
