...
In ESP8266 C++ development (Arduino core or SDK),
ICACHE_FLASH_ATTR is a function attribute macro that tells the compiler to store the function in flash memory
instead of IRAM (Instruction RAM).

1. Why it exists
	The ESP8266 has very limited IRAM (~32 KB for code).
	By default, some functions may be placed in IRAM for speed.
	If a function is not time-critical (e.g., not in an ISR), you can mark it with ICACHE_FLASH_ATTR so it stays in flash, saving IRAM space.

2. When NOT to use it
	Interrupt Service Routines (ISRs) must run from IRAM because flash access is disabled during certain operations.
	For ISRs, use ICACHE_RAM_ATTR (or IRAM_ATTR in newer cores).

3. Modern note
	In newer ESP8266 Arduino cores, most functions are stored in flash by default, so ICACHE_FLASH_ATTR is often unnecessary 
	unless you are working directly with the non-OS SDK or RTOS SDK.

4.  Rule of thumb:
	Normal functions → default (flash)
	Time-critical / ISR functions → ICACHE_RAM_ATTR / IRAM_ATTR
